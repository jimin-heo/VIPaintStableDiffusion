...
class BaseModel(torch.nn.Module):
    ...
    def apply_model(self, x, sigma, c_crossattn=None, y=None, skip_layers=[], controlnet_cond=None):
        ...
        model_output = checkpoint(self.diffusion_model, 
            x.to(dtype),
            timestep,
            use_reentrant=False,
...
def forward_sample(x, sigma_s, sigma_t):
    """Performs a single forward sampling step in the diffusion process."""
    alpha_t = (1. - sigma_t)
    alpha_s = (1. - sigma_s)
    ...
    sigma = torch.sqrt(v_ts)
    return x * append_dims(alpha_ts, x.ndim) + append_dims(sigma, x.ndim) * torch.randn_like(x)

def reverse_sample(x, sigma_t, sigma_s, denoised):
    """Performs a single reverse sampling step in the diffusion process."""
    alpha_t = (1. - sigma_t)
    alpha_s = (1. - sigma_s)
    alpha_ts = (alpha_t / alpha_s) 
    v_t = sigma_t ** 2
    ...
    v = v_ts * v_s / v_t
    sigma = torch.sqrt(v)
    mu = denoised * append_dims(alpha_s * v_ts / v_t, denoised.ndim) + x * append_dims(alpha_ts * v_s / v_t, x.ndim)
    return mu + append_dims(sigma, x.ndim) * torch.randn_like(x)

def forward_step(x, sigma_s, sigma_t):
    """Computes mean and std. dev for a forward step in the diffusion process."""
    alpha_t = (1. - sigma_t)
    alpha_s = (1. - sigma_s)
    alpha_ts = (alpha_t / alpha_s) 
    v_t = sigma_t ** 2
    ...
    sigma = torch.sqrt(v_ts)
    return x * append_dims(alpha_ts, x.ndim), append_dims(sigma, x.ndim)

def reverse_step(z_t, sigma_t, sigma_s, denoised, sigma_d=0.0):
    """Computes mean and std. dev for a reverse step in the diffusion process."""
    alpha_t = (1. - sigma_t)
    alpha_s = (1. - sigma_s)
    alpha_d = (1. - sigma_d)

    alpha_ts = (alpha_t / alpha_s) 
    alpha_sd = (alpha_s / alpha_d)
    ...
    v_ts = v_t - (alpha_ts ** 2) * v_s
    v_sd = v_s - (alpha_sd ** 2) * v_d
    p = (1. / v_sd + (alpha_ts ** 2) / v_ts)
    v = 1. / p
    sigma = torch.sqrt(v)
    mu = append_dims(p, z_t.ndim) * (append_dims(alpha_sd/ v_sd, denoised.ndim) * denoised + z_t * append_dims(alpha_ts / v_ts, z_t.ndim))
    return mu, sigma

def kl_divergence(mu, sigma, prior_mu, prior_sigma):
    """Calculates the KL divergence between two Gaussian distributions."""
    return 0.5 * (((mu - prior_mu) ** 2 + sigma ** 2) / (prior_sigma ** 2) - 2 * (torch.log(sigma) - torch.log(prior_sigma)) - 1)

def neg_log_prob(x, mu, sigma):
    """Calculates the log probability of a Gaussian distribution."""
    return 0.5 * (torch.log(2 * math.pi) + torch.log(sigma ** 2) + ((x - mu) ** 2) / (sigma ** 2))

def entropy(sigma):
    """Calculates the entropy of a Gaussian distribution."""
    return 0.5 * (torch.log(2 * math.pi) + torch.log(sigma ** 2) + 1)

def kl_divergence_diffusion(denoiser, mu, sigma, noise, t, sigmas, extra_args={}):
    z = mu + sigma * noise
    t = t * torch.ones([z.shape[0]], device=z.device, dtype=torch.float32)
    t_mid = torch.randint(size=t.shape, device=t.device, dtype=torch.int64, low=-1, high=t)

    is0 = t_mid < 0
    ist = t_mid == (t - 1)
    t_mid = torch.where(is0, t_mid + 1, t_mid)
    mu_mid, sigma_mid = forward_step(z, sigmas[t], sigmas[t_mid])

    denoised = denoiser(z_mid, sigmas[t_mid], **extra_args)
    mu_prior, sigma_prior = reverse_step(z_mid, sigmas[t_mid], sigmas[t_mid+1], denoised)
    mu_post, sigma_post = reverse_step(z_mid, sigmas[t_mid], sigmas[t_mid+1], z, t)

    kl = kl_divergence(mu_post, sigma_post, mu_prior, sigma_prior)
    kl_t = neg_log_prob(z, mu_prior, sigma_prior) - entropy(sigma)
    kl_t0 = kl_divergence(mu_mid, sigma_mid, torch.zeros_like(mu_mid), torch.ones_like(sigma_mid))
    
    kl = torch.where(is0, kl_t0, kl)
    kl = torch.where(ist, kl_t, kl)
    return kl, z

@torch.no_grad()
@torch.autocast("cuda", dtype=torch.float16)
def sample_ancestral(model, x, sigmas, extra_args=None):
    print('COND SCALE', extra_args['cond_scale'])
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    for i in tqdm(range(len(sigmas) - 1)):
        denoised = model(x, sigmas[i] * s_in, **extra_args)
        x = reverse_step(
            x, sigmas[i], sigmas[i + 1], denoised
        )
    return x
