...
from sd3_impls import (
    ...
    CFGDenoiser,
    SD3LatentFormat,
    SkipLayerCFGDenoiser, kl_divergence, reverse_step, forward_step, reverse_sample, forward_sample
)
...
class SD3:
    def __init__(
        ...
        with safe_open(model, framework="pt", device="cpu") as f:
            ...
            self.model = torch.compile(BaseModel(
                shift=shift,
                file=f,
...
class SD3Inferencer:
    ...
    def load(
        ...
        self.print(f"Loading SD3 model {os.path.basename(model)}...")
    ...
    #     latent = SD3LatentFormat().process_in(tensor)
    #     return latent
    ...
    def encode_first_stage(self, image: torch.Tensor, vae_cpu: bool=True) -> torch.Tensor:
        """Encodes the image tensor into a latent representation.
            Assumes the image is in the range [0, 1] and has shape (C, H, W).
        """
        if image.dim() == 3:
            image = image.unsqueeze(0)
        image = image * 2.0 - 1.0  # Scale to [-1, 1]
        image_torch = image.cuda()
        self.vae.model = self.vae.model.cuda()
    ...
    def decode_first_stage(self, latent: torch.Tensor, vae_cpu: bool=False) -> torch.Tensor:
        """Decodes the latent representation back into an image tensor.
            Assumes the latent is in the format expected by the VAE.
        """
        latent = SD3LatentFormat().process_out(latent)
        self.vae.model = self.vae.model.cuda()
        image = self.vae.model.decode(latent)
    ...
    def gen_image(
        ...
        for i, prompt in pbar:
            ...
            self.print("Getting latent for prompt:", prompt)
            conditioning = self.get_cond(prompt)
            self.print("Conditioning done")
            self.print("Sampling latent...")
            print('cfg_scale', cfg_scale)
            print('CFG_SCALE', CFG_SCALE)
            ...
            self.print("Sampling done")
            self.print("Decoding sampled latent to image...")
...
@torch.no_grad()
def load_model(model, skip_layer_cfg=False):
    ...
    config = CONFIGS.get(os.path.basename(model), {})
    ...
    inferencer.cfg_scale = config.get("cfg", 5.0)
